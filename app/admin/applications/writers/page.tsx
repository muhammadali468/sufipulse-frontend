...
export default function AdminWriterApplications() {
    ...
    //   async function getCurrentUserPublicId(): Promise<string | null> {
    //     if (!user) return null;
    ...
    //       return data?.id || null;
    //     } catch (error) {
    //       console.error('Error in getCurrentUserPublicId:', error);
    //       return null;
    //     }
    //   }
    ...
    //       const reviewerId = await getCurrentUserPublicId();
    ...
    //         await assignWriterRole(selectedApp.user_id);
    //       }

    //       // Send notification to user about status change
    //       if (selectedApp) {
    //         const userEmail = selectedApp.email || selectedapp.email || '';
    //         const submissionReference = `SP-WRT-${new Date().getFullYear()}-${selectedApp.id.substring(0, 8).toUpperCase()}`;

    //         try {
    //           // Create submission tracking if it doesn't exist
    //           const { data: existingTracking } = await supabase
    //             .from('submission_tracking')
    //             .select('id')
    //             .eq('submission_id', applicationId)
    //             .eq('submission_type', 'writer_application')
    //             .maybeSingle();

    //           if (!existingTracking) {
    //             await createSubmissionTracking({
    //               submissionId: applicationId,
    //               submissionType: 'writer_application',
    //               submissionReference,
    //               userId: selectedApp.user_id || undefined,
    //               userEmail,
    //               submitterName: selectedApp.pen_name,
    //               currentStatus: status,
    //               submissionData: {
    //                 pen_name: selectedApp.pen_name,
    //                 bio: selectedApp.bio?.substring(0, 100) + '...'
    //               }
    //             });
    //           }
    ...
    //         approved: 'Application approved successfully! Writer role has been assigned. User has been notified.',
    //         rejected: 'Application rejected. User has been notified.',
    //         under_review: 'Application marked as under review. User has been notified.',
    //         revision_requested: 'Revision requested for this application. User has been notified.',
    //       };
    ...
    //   async function assignWriterRole(userId: string) {
    //     try {
    //       const { data: writerRole, error: roleError } = await supabase
    //         .from('roles')
    //         .select('id')
    //         .eq('name', 'writer')
    //         .maybeSingle();
    ...
    //       if (!writerRole) {
    //         console.error('Writer role not found in database');
    //         return;
    //       }
    ...
    //         .select('id')
    //         .eq('user_id', userId)
    //         .eq('role_id', writerRole.id)
    //         .maybeSingle();

    //       if (existing) {
    //         console.log('User already has writer role');
    //         return;
    //       }
    ...
    //       console.log('Writer role assigned successfully');
    //     } catch (error) {
                        ...
                        <div className="dashboard-modal max-w-4xl relative" onClick={(e) => e.stopPropagation()}>
                                ...
                                <div className="space-y-6">
                                    {/* <div className="grid grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <p className="text-[var(--dash-text-primary)] font-medium">{selectedApp.pen_name}</p>
                                        </div>

                                        <div>
                                            <label className="dashboard-label">Current Status</label>
                                            <span className={`${getStatusBadgeClass(selectedApp.profile_status)} flex items-center gap-1 w-fit`}>
                                                {getStatusIcon(selectedApp.profile_status)}
                                                {selectedApp.profile_status.replace(/_/g, ' ')}
                                            </span>
                                        </div>
                                    </div> */}
                                    ...
                                    {/* <div>
                                        ...
                                        <div className="bg-[var(--dash-bg-secondary)] rounded-lg p-4 border border-[var(--dash-border)]">
                                            <p className="text-[var(--dash-text-secondary)] whitespace-pre-wrap">{selectedApp.bio}</p>
                                        </div>
                                    </div> */}
                            ...
                            <div className="dashboard-modal-footer">
                                <div className="grid grid-cols-4 gap-3 w-full">
                                    ...
                                    <button
                                        // onClick={() => updateApplicationStatus(selectedApp.id, 'revision_requested')}
                                        disabled={processingAction}
